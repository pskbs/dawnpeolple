...
7. **동의 항목 중 "생일이 선택인지 필수로 강제 설정 가능한지"는 이번 조사로 명확한 답을 얻지 못함.** 콘솔에서 요청 스코프 자체는 서비스가 고를 수 있는 것으로 보이나(예: USER_BIRTHDAY를 요청 목록에 포함할지 여부는 서비스 선택), 그 항목을 사용자에게 "필수 동의"로 강제할 수 있는지, 아니면 항상 "선택 동의"로만 노출되는지에 대한 명시적 문구를 확보하지 못함. → 콘솔 화면에서 직접 확인 필요.
