...
- 벙개모임: **누구나 개설**(신고·차단·개설 한도 필수). **개설 지역은 전국 오픈**(2026-09-22 결정, docs/decisions.md — 부천 고정 바인딩 해제). 개설 시 시/도·시/군/구 선택 + 동/읍/면·구체 장소(역·동 단위, 가게명까지 유도 안 함)는 선택 입력. 목록은 회원이 설정한 동네 기준 **가까운 순 정렬**(지역 필터링은 아님, 전국이 다 보여요).
- 수익: 앱인토스 인앱 광고. 배포: 앱인토스 → 웹(Vercel) → (추후) 스토어 앱.
- 지역: 수다방·소모임 모두 **전국 오픈**. 회원은 가입 시 시/도·시/군/구를 설정하고, "내 동네"를 최대 3개(이름 지정 가능 — 집/회사 등)까지 저장해 전환할 수 있음(`src/features/location/LocationSheet.tsx`). 부천은 서비스 시작 지역일 뿐 코드상 지역 잠금은 없음(`src/config/regions.ts`).
...
- 광고: 글·벙개 상세 배너만(2026-09-24 전면 광고 제거 — docs/decisions.md). 피드 목록 사이 광고 없음. 리워드 제외.
